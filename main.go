...
using the Tillitis TKey. The message will be pre-hashed using
SHA512 and signed with Ed25519ph using the TKey's private key.
...
Use <command> --help for further help, i.e. %[1]s verify --help`, os.Args[0])
  ...
  pre-hashed using SHA512 and signed with Ed25519ph using the TKey's private key.
  ...
  Verifies wheather the Ed25519ph signature of the message is valid
  using the public key. Does not need a connected TKey to verify.

  SIG-FILE is expected to be an 64 bytes Ed25519ph signature in hex.
